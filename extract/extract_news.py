...
def news_extractor_per_media(
    country: str, medias: dict, today: datetime, t_delta: timedelta, conn
):
    # query
    cursor = conn.cursor()
    ...
    for media in medias_urls:
        try:
            media_news = newspaper.build(
                media, memoize_articles=False, number_threads=4
            )
        ...
        for art in media_news.articles:
            try:
                ...
                if (
                    ...
                    and art.publish_date >= (today - t_delta)
                    and art.url not in saved_articles_urls
                ):
                    media_name = (
                    ...
                    date = (
                        art.publish_date.strftime("%Y-%m-%d %H:%M:%S")
                        if art.publish_date
                        else None
                    )
...
def news_extractor_per_media_concurrent(
    media: str, today: datetime, t_delta: timedelta, conn
):
    # query
    saved_articles_urls = set(
        [
            article[0]
            for article in conn.execute(f"""SELECT url FROM news_chile""").fetchall()
        ]
    )
    # scrapings
    try:
        ...
        for art in media_news.articles:
            try:
                ...
                print(
                    f"Ha ocurrido un error al descargar un artículo de {media}:\n\t{e2}"
                )
                continue
            else:
                conn.execute(
...
def get_news_from_table_and_media(
    connection,
    table_name: str = "news_chile",
    n: int = 10,
    delta_days=1,
    media_name: str = None,
):
...
def get_news_from_table(
    connection: psycopg.Connection, table_name: str = "news_chile", delta_days: int = 1
):
    ...
    if table_name:
        ...
        raise ValueError("Argument `table_name` needed.")
    try:
        results = connection.execute(query).fetchall()
        for news_data in results:
            news_id, news_date, news_media, news_url, news_text, news_keywords = (
                news_data
            )
            yield (news_id, news_date, news_media, news_url, news_text, news_keywords)
    except Exception as e:
        print(f"An error has ocurred: {e}")
